 ...
 */
export interface PreviousChapter { chapterNumber: number; title: string; content: string; chosenOption?: string; }
export interface GenerateStoryPayload {
  storyId?: string; storyTitle: string; storyAuthor: string;
  storySynopsis: string; storyTags?: string[]; previousChapters: PreviousChapter[];
  chosenFate: { option: 'A' | 'B'; text: string; isForceChoice: boolean };
  chapterNumber: number;
}
export interface GenerateStoryResult { title: string; content: string; optionA: string; optionB: string; }
...
export async function generateStoryChapter(
  ...
  if (!payload.storyTitle || !payload.chosenFate) throw new Error('storyTitle ve chosenFate zorunludur');
  const { callGenerateStory } = await import('@/lib/functions-client');
  try {
    return await callGenerateStory({ ...payload, operationId, action });
  } catch (err: unknown) {
    if (err instanceof Error) {
      const msg = err.message; const colonIdx = msg.indexOf(': ');
